...
describe Commands::HandleInitializationExceptions do
  ...
  it 'handles initialization errors' do
    ...
    described_class.call(io_adapter: io_adapter) { raise Errors::InvalidSceneReference, 'blabla' }
    ...
    described_class.call(io_adapter: io_adapter) { raise Errors::StoryNotCompletable }

    expect(io_adapter).to receive(:println).with('Story file: Not found. Make sure to specify an existing file')
    described_class.call(io_adapter: io_adapter) { raise Errors::StoryFileNotFound }
  end
end
